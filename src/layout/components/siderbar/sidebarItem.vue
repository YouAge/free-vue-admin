...
<template>
  ...
  >
      <template v-if="parentItem.children && parentItem.children.length">
        <sidebar-item v-for='item in parentItem.children'
          :parent-item="item" :key="item.name"
          :base-path='handlePath(item.path)'></sidebar-item>
      </template>
  </component>
</template>
...
export default {
  ...
  computed:{
    currentMenus(){
      ...
      if(this.handleChildren(this.parentItem.children, this.parentItem) && (
        this.routeChildren)){
        return 'freeMenuItem'
      }else {
        return 'freeSubMenu'
